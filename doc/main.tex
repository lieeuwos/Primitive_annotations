...
In figure \ref{fig:BiasScaling} the bias percentage is given against the instance percentage of a dataset. The averaged dataset has 3186 instances and 160 features.
...
Within saving as many information obtainable from the algorithms a missed opportunity is the probability prediction of all the test sets when a classifier uses probability to predict. 
...
The bias error is high for all classifiers this is mostly due to a low variance error. The naive bayes, GaussianNB and BernoulliNB and SVC-rbf do take the throne for most biased solutions. The results from these classifier is to be expected as their focus is a generalization of the dataset to fit their solution. The least biased solutions SGDClassifier and RandomForestClassifier gain their lack of bayes by being depended on randomness for classification. The other classifiers like GradientBoostingClassifier and AdaBoost show already a large increase in bias based on an increase in the dataset, matching previous results. 
...
\bibitem{hyperpara} An emperical study of hyperparameter importance across datasets, Jan van Rijn, Frank Hutter, Proc. of AutoML (2017): 97-104.
...
figures \ref{fig:Overfit} - 11, 12, 37, 54, 458, 1038, 1043
...
\begin{figure}[h] 
	...
	\caption{Duration of the oldest version of adding random(dotted) or duplicate features(default)}
	\label{fig:durOld}
\end{figure}
