...
\textbf{SVC-rbf} \\
\textbf{GaussianNB} On the smallest duration \\
\textbf{BernoulliNB} Removing the least important features from a dataset has less of an impact of decreasing the duration than removing random features. This can be due to the randomly remaining features being easier to binarize and quicker to calculate mean and variance of. \\

\textbf{GradientBoostingClassifier} As the only classifer GradientBoostingClassifier takes less time with only the most important features. This can be the nature of the stagewise steps of GradientBoostingClassfier needing less time with more successfull features than random features whos importance can be questionable. The dataset with the least correlated features hardly as any difference with the least important removed feature dataset.\\


\newpage

\paragraph{combined}
in figure \ref{fig:Log} adding and removing features is shown.
in figure \ref{fig:FeatManSlope} the slope is shown.
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{features removed randomly and features   duplicatedd with a log scale of \ref{fig:FeatManLog}}
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{features removed randomly and features   duplicatedd with a log scale}
		\label{fig:FeatManSlope1Log}
	\end{subfigure}
	
	\caption{Slopes of Feature percentage total duration of the original dataset}
